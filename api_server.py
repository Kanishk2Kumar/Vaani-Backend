...
async def create_assistant(
    ...
    try:
        ...
        supabase.table("chat_history").insert({
            "session_id": "1",
            "assistant_id": assistant_id,
            "user_query": first_message,
            "bot_response": "",  # Leave empty if not generated yet
            "created_at": created_at
        }).execute()
...
async def voice_chat_with_agent(
    ...
    """
    Unified voice chat endpoint: audio → transcription → context → response
    Returns text response ready for TTS (no translation)
    """
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=401, detail=f"Transcription error: {str(e)}")
        finally:
        ...
        # Step 4: Generate response in the requested language
        language_prompt = ""
        if language == "hi":
            language_prompt = """
        ...
        bot_response = generate_response(
            ...
            language=language  # Generate in the requested language directly
        )

        # Step 5: Store in Supabase (only final response)
        supabase.table("chat_history").insert({
            "session_id": session_id,
